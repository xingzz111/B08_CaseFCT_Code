...
class FixtureHandler:
    ...
    IN1 = FIXTURE_CFG.get("cmd", {}).get("mid", "fixture_in_half")
    ...
    IN_OK = "fixture_in [OK]"
    IN1_OK = "fixture_in_half [OK]"
    DOWN_OK = "fixture_down [OK]"
    RUN_OK = "fixture_run [OK]"
    INSERT_OK = "fixture_uninsert [OK]"
    UP_OK = "fixture_up [OK]"
    FIX_OK = "fix ok!"
    ...
    def run(self):
        ...
        while self.serving:
            ...
            if message:
                ...
                # A = select.select([self.fixtureUart], [], [], 1)
                # print(A)
                # rList, _, _ = A
                #
                # if not rList:
                ...
                if reply:
                    ...
                    if self.isTesting and FixtureHandler.RUN_OK in reply:
                        # time.sleep(3)
                        continue
                    # if FixtureHandler.MID_OK in reply:
                    #     scan_flag, sn = self.scan_sn()
                    #     # scan_flag = True
                    #     # sn = "301WT24G1F001V"
                    #     time.sleep(1)
                    #     if scan_flag:
                    #         event = '[AutoScan]:' + sn
                    #     else:
                    #         event = '[Message]:scan barcode fail --> ' + sn
