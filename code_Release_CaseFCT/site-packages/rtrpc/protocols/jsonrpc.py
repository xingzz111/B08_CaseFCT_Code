...
class JSONRPCServerStopError(Exception):  # pragma: no cover
    jsonrpc_error_code = -32604
    message = "Server Stop Requested"


class JSONRPCServerWorkerUnavailableError(Exception):
...
class JSONRPCServerError(Exception):  # pragma: no cover
    ...
    def __init__(self, msg='', code=None):
        ...
        if msg:
            self.message = msg
        if code:
            self.jsonrpc_error_code = code


class JSONRPCPluginError(Exception):  # pragma: no cover
...
class JSONRPCErrorResponse(RPCResponse):
    ...
    def to_exception_obj(self):
        return JSONRPCServerError(self.error, self._jsonrpc_error_code)
...
class JSONRPCRequest(RPCRequest):
    def error_respond(self, error):
        ...
        # make sure all RPC internal error has header [RPCError]
        response.error = '[RPCError] {}'.format(error.message if hasattr(error, 'message') else str(error))
...
class JSONRPCProtocol(RPCBatchProtocol):
    ...
    def error_respond(self, error, original_request):
        ...
        if not hasattr(error, 'jsonrpc_error_code'):
            error = JSONRPCServerError(error)
