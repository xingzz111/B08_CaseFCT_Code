        ...
        for i in range(self.retries):
            ...
            if reply:
                ...
                if response.unique_id != req.unique_id:
                    self.publisher.publish('Reply ID {0} is not matching Request ID {1}'.format(req.unique_id, response.unique_id), level=levels.CRITICAL)
                    return None
                ...
                self.publisher.publish(
                    'timed out waiting for response to req['+req.unique_id+'], retries='+str(self.retries-i-1), level=levels.INFO)
        return None
