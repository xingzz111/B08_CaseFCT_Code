...
class ScanController(QObject):
    ...
    def startLoop(self):
        ...
        self.view.buttonLoop.setText("Loop Out(Vòng lặp ra)")
        self.view.buttonStart.setEnabled(False)
        
    def stopLoop(self):
        self._looping = False
        self.view.buttonLoop.setText("Loop In(Vòng lặp vào)")
    ...
    def nextCycle(self):
        self._timer.stop()
        flag = self.loopAction != "Normal (Bình thường)"
        self._signalLoop.emit(flag)

    def login(self):
        user = "Operator"
        if self.view.buttonLogin.text() == "Login (Đăng nhập)":
    ...
    def changeMode(self, user):
        if user == "Administrator":
            self.view.buttonLogin.setText("Logout (Đăng xuất)")
            self.view.labCurrUser.setText("Administrator (Quản trị viên)")
            ...
            self.view.buttonLogin.setText("Login (Đăng nhập)")
            self.view.labCurrUser.setText("Operator (Người điều hành)")
            self.view.labCurrUser.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
            self.view.buttonLoad.setEnabled(False)
            self.view.buttonReLoad.setEnabled(False)
            self.view.buttonLoop.setEnabled(False)
...
class ScanView(QFrame):
    ...
    def controlLayout(self):
        ...
        self.labCurrUser = QLabel("Administrator (Quản trị viên)")
        self.labCurrUser.setFixedSize(90, 30)
        self.labCurrUser.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self.buttonLogin = QPushButton("Logout (Đăng xuất)")
        ...
        self.buttonLoad = QPushButton("Load (rọng tải)")
        self.buttonLoad.setShortcut(QKeySequence(Qt.CTRL | Qt.Key_L))
        self.buttonReLoad = QPushButton("MES (mạng lưới mes)")
        self.buttonReLoad.setShortcut(QKeySequence(Qt.CTRL | Qt.Key_R))
        # Added: Step Debug button
        self.buttonStepDebug = QPushButton("StepDebug (Gỡ lỗi từng bước)")
        ...
        self.buttonLoop = QPushButton("Loop In(Vòng lặp vào)")
        self.buttonLogPath = QPushButton("TestLog (Nhật ký thử nghiệm)")
        self.stopOnFail = QPushButton("autoscan (tự động quét)")
        ...
        labLoopCount = QLabel("LoopCount (Số lần lặp lại)")
        ...
        labLoopDuration = QLabel("LoopDuration (Thời lượng vòng lặp)(ms)")
        self.lineEditLoopDuration = QLineEdit("5000")
        self.lineEditLoopDuration.setValidator(numValidator)
        labLoopAction = QLabel("LoopAction (Hành động lặp lại)")
        self.selectLoopAction = QComboBox()
        self.selectLoopAction.addItems(["Normal (Bình thường)", "NoAction (Không có hành động nào)"])
        self.selectLoopAction.setCurrentText("Normal (Bình thường)")
    ...
    def yieldLayout(self):
        ...
        labPass = QLabel("PASS (VƯỢT QUA):")
        labFail = QLabel("FAIL (THẤT BẠI):")
        labTotal = QLabel("Total (Tổng cộng):")
        ...
        lbl_fail_1 = QLabel("Tested (Đã được kiểm tra): ")
    ...
    def snScanLayout(self):
        ...
        self.buttonStart = QPushButton("Start (Bắt đầu) (F5)")
        self.buttonStart.setShortcut(QKeySequence(Qt.Key_F5))
        self.buttonStop = QPushButton("Stop (dừng lại)")
