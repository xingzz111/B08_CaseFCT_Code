...
class MainController(BaseView):
    ...
    def sequence_start(self, site):
        ...
        if self.scanC.looping and self.scanC.loopAction == "NoAction (Không có hành động nào)":
    ...
    def sequence_finish(self, result):
        ...
        if constant.SIMULATE:
            return
        elif self.scanC.looping and self.scanC.loopAction == "NoAction (Không có hành động nào)":
            ...
            for k,v in enumerate(sn_list):
                if self.scanC.slotsC.currState(k) not in ("DISABLE", "READY", "RUNNING"):
                    if v == "None" or len(v) != 23:
                        self.messageBox(f"SLOT{k} SN: '{v}' is invalid! (SN không hợp lệ)", levels.CRITICAL)
    ...
    def stop_on_fail(self):
        ...
        if not self.processes.check_stoponfail_status():
            ...
            self.scanC.view.stopOnFail.setText("autoscan (tự động quét)")
            # self.heaterC.view.enable_pdca(False)
        else:
            self.processes.stop_on_fail(False)
            self.scanC.view.stopOnFail.setText("manualscan (quét thủ công)")
            # self.heaterC.view.enable_pdca(True)
    ...
    def loopAction(self):
        if self.scanC.looping:
            ...
            if not self.scanC.isReady():
                self._signalMessageBox.emit(f"Please scan SN first (Vui lòng quét số sê-ri trước.)!", levels.WARNING)
                return
